...
(function () {
  ...
  var I18N = {
    ...
    'gal.note':   { ja: '画像はどれも、左のソースをこのプレビューアで実際に描画したものです（<code>tools/preview-harness/shoot-docs.py</code> が生成しています）。カレンダーの祝日はオンラインのときだけ取得します（取れなければ祝日なしで描きます）。縦書き・文庫組みの例は上の「文庫のかたちで、書く。」を、スライドの例は「Markdown のまま、発表する。」をご覧ください。',
                    en: 'Every picture here is the source beside it, actually rendered by this previewer (generated by <code>tools/preview-harness/shoot-docs.py</code>). Calendar holidays are fetched only when online (without a connection it simply draws none). For vertical paperback typesetting see “Write it like a paperback” above, and for slides see “Present straight from Markdown”.' },

...
(function () {
  ...
  var I18N = {
    ...
    'gal.note':   { ja: '画像はどれも、左のソースをこのプレビューアで実際に描画したものです（<code>tools/preview-harness/shoot-docs.py</code> が生成しています）。縦書き・文庫組みの例は上の「文庫のかたちで、書く。」を、スライドの例は「Markdown のまま、発表する。」をご覧ください。',
                    en: 'Every picture here is the source beside it, actually rendered by this previewer (generated by <code>tools/preview-harness/shoot-docs.py</code>). For vertical paperback typesetting see “Write it like a paperback” above, and for slides see “Present straight from Markdown”.' },
    ...
    'gal.colAlt':    { ja: '左右 2 カラムに分かれた見出しと本文',
                       en: 'A heading and body split into two columns' },

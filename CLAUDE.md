...
- Mermaid diagrams, KaTeX math, syntax-highlighted code blocks with copy button, **Zukai (図解) diagrams** (`zukai` codeblock — fishbone, mandala chart, BMC, flow, cycle; in-house, renamed from the working title "smartart" to avoid conflict with Microsoft's SmartArt trademark; named for the Japanese term 図解 "diagrammatic explanation"), CSV/TSV codeblocks → tables, **Plotly charts from external CSV/TSV** (`plotly` codeblock with `file:` directive), **Sakuzu (作図) elementary geometry diagrams** (`sakuzu` codeblock — points / segments / circles / triangles / derived points / transformations / angle marks; 2D only, 3D reserved for the future; in-house, Eukleides-inspired but with no GPL code reused; named for the Japanese term for "geometric construction" to avoid collision with the unrelated npm package `euclid.js`), Markdown footnotes — see *Preview Pipeline*
...
- **Mermaid / Zukai** — ` ```mermaid ` and ` ```zukai ` fenced blocks are intercepted post-`marked.parse()` and substituted with rendered SVG. Zukai (図解, "diagrammatic explanation"; renamed from the working title "smartart" to avoid conflict with Microsoft's SmartArt trademark) supports fishbone, mandala chart, BMC (Business Model Canvas), flow, and cycle diagrams via the bundled `zukai.js`. All Zukai types accept an optional `scale: <factor>` directive alongside `title:` (e.g., `scale: 0.7` shrinks to 70%, `scale: 1.5` enlarges to 150%); applied by `applyScale()` in `assets/libs/zukai.js`, which post-processes the root `<svg>`'s `width`/`height` attributes and strips `max-width:100%` from the inline style when the factor exceeds 1 so the diagram can overflow the container.
...
- **Bundled starter themes**:
  ...
  - `tategaki.css` — Japanese vertical-writing theme (`writing-mode: vertical-rl`, mincho fonts, kinsoku 禁則 / tate-chu-yoko 縦中横 / palt 約物詰め, with `pre` / `table` / `img` / math / mermaid / zukai kept as horizontal islands; intentionally color-agnostic so it composes with both Light and Dark).
...
- **Extension inheritance inside slides**:
  ...
  - ` ```mermaid `, ` ```zukai `, and ` ```csv ` / ` ```tsv ` code blocks — post-render DOM substitution, reusing the same renderers as the regular pipeline.
  - `highlight.js` for syntax-coloring.
  - KaTeX math and Marp directives like `paginate` work out of the box.
- **Figure auto-centering** — when a non-split slide contains a `.mermaid` / `.zukai` / `.csv-table` / native `<table>` / image-only `<p>` (a paragraph whose sole meaningful child is `<img>`) as a direct child of `<section>`, a post-pass in `renderMarp()` wraps that element and every following sibling into `<div class="marp-figure-stage">` and tags the section with `marp-has-figs`. CSS then keeps the section a flex column (matching marp themes' own layout), pins `justify-content` to `flex-start` so leading text (heading + paragraphs) stays top-aligned, and gives the stage `flex: 1 1 auto` + inner `justify-content: center` so the figure(s) sit vertically centered within the remaining space (gap 16px). Horizontal centering is per-element (`text-align: center` on the mermaid/zukai/image wrapper, `margin: 0 auto` on tables/csv-tables) instead of `align-items: center` on the stage — `align-items: center` collapses the mermaid/zukai wrapper width and shrinks the inline SVG. Designed for the common "title + 1 line of context + diagram/table/image" layout — any content *after* the first figure also moves into the centered stage, so authors should keep the "text → figure" order. Split slides (`split` / `split-2` / `split-3` / `split-4`) are excluded since they use grid layout.
- **Column layout** (`<!-- _class: split -->` / `split-2` / `split-3` / `split-4`) — break a slide into 2 / 2 / 3 / 4 equal-width columns. Authors separate columns with a `+++` line at the start of a line; a `renderMarp()` pre-pass replaces each `+++` with a `⦃COL⦄` sentinel paragraph (skipping fenced code blocks so e.g. ```diff stays intact), and a post-pass groups direct children of `section[class*="split"]` into `<div class="split-col">` buckets at every sentinel `<p>`. Leading `<h1>` / `<h2>` of the first bucket are pulled out as a full-width header row (`grid-column: 1 / -1`) so all columns start at the same y. CSS applies `display: grid` with `repeat(N, 1fr)` and per-class gap. Runs after footnote restoration (so `<sup>` refs stay in their column) and before mermaid / zukai / csv / plotly substitution (so those land inside the correct column). The figure-centering pass explicitly skips these sections.
...
- `assets/libs/` — bundled JS/CSS: `marked`, `mermaid`, `katex/`, `highlight.js/`, `zukai.js`, `marp/` (Marp Core IIFE), `editor/` (CodeMirror 6 + Vim IIFE), `plotly/plotly.min.js` (Plotly.js full dist-min), `js-yaml.min.js` (YAML spec parser for `plotly` blocks).

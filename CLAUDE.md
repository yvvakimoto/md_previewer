...
- **Figure auto-centering** — when a non-split slide contains a `.mermaid` / `.smartart` / `.csv-table` / native `<table>` / image-only `<p>` (a paragraph whose sole meaningful child is `<img>`) as a direct child of `<section>`, a post-pass in `renderMarp()` wraps that element and every following sibling into `<div class="marp-figure-stage">` and tags the section with `marp-has-figs`. CSS then keeps the section a flex column (matching marp themes' own layout), pins `justify-content` to `flex-start` so leading text (heading + paragraphs) stays top-aligned, and gives the stage `flex: 1 1 auto` + inner `justify-content: center` so the figure(s) sit vertically centered within the remaining space (gap 16px). Horizontal centering is per-element (`text-align: center` on the mermaid/smartart/image wrapper, `margin: 0 auto` on tables/csv-tables) instead of `align-items: center` on the stage — `align-items: center` collapses the mermaid/smartart wrapper width and shrinks the inline SVG. Designed for the common "title + 1 line of context + diagram/table/image" layout — any content *after* the first figure also moves into the centered stage, so authors should keep the "text → figure" order. Split slides (`split` / `split-2` / `split-3` / `split-4`) are excluded since they use grid layout.
- **Column layout** (`<!-- _class: split -->` / `split-2` / `split-3` / `split-4`) — break a slide into 2 / 2 / 3 / 4 equal-width columns. Authors separate columns with a `+++` line at the start of a line; a `renderMarp()` pre-pass replaces each `+++` with a `⦃COL⦄` sentinel paragraph (skipping fenced code blocks so e.g. ```diff stays intact), and a post-pass groups direct children of `section[class*="split"]` into `<div class="split-col">` buckets at every sentinel `<p>`. Leading `<h1>` / `<h2>` of the first bucket are pulled out as a full-width header row (`grid-column: 1 / -1`) so all columns start at the same y. CSS applies `display: grid` with `repeat(N, 1fr)` and per-class gap. Runs after footnote restoration (so `<sup>` refs stay in their column) and before mermaid / smartart / csv / plotly substitution (so those land inside the correct column). The figure-centering pass explicitly skips these sections.

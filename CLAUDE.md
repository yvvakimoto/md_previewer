...
- Mermaid diagrams, KaTeX math, syntax-highlighted code blocks with copy button, **Schemata diagrams** (`schemata` codeblock — fishbone, mandala chart, BMC, flow, cycle; in-house; renamed from earlier working titles "smartart" → "zukai" → "schemata" — "smartart" collided with Microsoft's SmartArt trademark, "zukai" was changed for naming consistency with the sibling kataskeve module; "schemata" is the Greek plural of σχῆμα "figure / form / template", reflecting the family of structured-explanation templates the module renders), CSV/TSV codeblocks → tables, **Plotly charts from external CSV/TSV** (`plotly` codeblock with `file:` directive), **Kataskeve elementary geometry diagrams** (`kataskeve` codeblock — points / segments / circles / triangles / derived points / transformations / angle marks; 2D only, 3D reserved for the future; in-house, Eukleides-inspired but with no GPL code reused; renamed from earlier working titles "euclid" → "sakuzu" → "kataskeve" — "euclid" collided with the npm package `euclid.js`, "sakuzu" collided with the Microsoft Store UWP app "Sakuzu" (a Windows diagram tool in the same product category); "kataskeve" is κατασκευή, the term Euclid used in the Elements for the *construction step* of a proposition), Markdown footnotes — see *Preview Pipeline*
...
**Diagram render memoization** — because `#preview` is rebuilt via `innerHTML` on every render (including every live-edit keystroke, ~150ms debounced), mermaid / schemata / kataskeve diagrams are memoized so an unchanged diagram re-attaches its prior output instead of re-running the (expensive) renderer. Shared module-level caches (`__mermaidCache` / `__schemataCache` / `__kataskeveCache` in `assets/index.html`) are keyed by `(dark/light theme salt + source string)` via `__diagKey()` — all three renderers vary their output with `body.dark-mode`, so the theme salt is required for correct invalidation on the `M`-key toggle. The shared helpers `renderMermaidCached` / `renderSchemataCached` / `renderKataskeveCached` are wired into **both** `renderMarp()` and the normal `renderMarkdown()` async render phases (schemata/kataskeve go through each library's single-element `render(source)` API instead of `renderAll()` so each block memoizes individually). A FIFO cap (`__DIAGRAM_CACHE_MAX = 200`) bounds memory across long sessions; render errors are not cached. Plotly is excluded (live interactive widget).
...
- **Mermaid / Schemata** — ` ```mermaid ` and ` ```schemata ` fenced blocks are intercepted post-`marked.parse()` and substituted with rendered SVG. Schemata (σχήματα, Greek for "figures / forms / templates"; earlier working titles "smartart" → "zukai" both withdrawn — "smartart" collided with Microsoft's SmartArt trademark and "zukai" was changed for consistency with the kataskeve rename) supports fishbone, mandala chart, BMC (Business Model Canvas), flow, and cycle diagrams via the bundled `schemata.js`. All Schemata types accept an optional `scale: <factor>` directive alongside `title:` (e.g., `scale: 0.7` shrinks to 70%, `scale: 1.5` enlarges to 150%); applied by `applyScale()` in `assets/libs/schemata.js`, which post-processes the root `<svg>`'s `width`/`height` attributes and strips `max-width:100%` from the inline style when the factor exceeds 1 so the diagram can overflow the container.
...
- **Kataskeve (作図) geometry diagrams** — fenced ` ```kataskeve ` blocks contain an expression-oriented DSL inspired by (but not derived from) the GPL-licensed Eukleides tool. Earlier working titles "euclid" → "sakuzu" both withdrawn — "euclid" collided with the npm package `euclid.js`, "sakuzu" collided with the [Microsoft Store UWP app "Sakuzu"](https://apps.microsoft.com/detail/9P99DDZWNHKZ) (a Windows diagram tool in the same product category). The current name κατασκευή is the term Euclid used in the Elements for the *construction step* of a proposition. Original implementation in `assets/libs/kataskeve.js` (in-house IIFE; same module shape as Schemata). The parser is a recursive-descent expression evaluator over a small object model — Point / Segment / Line / Ray / Circle / Arc / Polygon / Vector. Built-ins cover primitive constructors (`point`, `segment`, `line`, `ray`, `circle`, `circle3`, `circle_diameter`, `arc`, `polygon`, `regular`), derived points (`midpoint`, `intersection` over line×line / line×circle / circle×circle, `foot`, `circumcenter`, `incenter`, `orthocenter`, `centroid`, `point_on(circle, deg)`), transformations (`translate`, `rotate`, `reflect`, `scale`), and triangle convenience forms (`triangle(P,Q,R)`, `triangle(a,b,c)` SSS, `triangle()` default scalene). A `.` member operator on a 3-vertex polygon exposes `T.A` / `T.B` / `T.C` plus `T.AB` / `T.BC` / `T.CA` so authors can name the vertices after a `T = triangle(5,4,3)` SSS call. Settings: `viewport: xmin ymin xmax ymax` (auto-bbox + 10% margin if omitted), `unit: <px-per-world-unit>` (default 40), `grid: on|off`, `axes: on|off`. `mode: 3d` is reserved for the future and currently a no-op. Bare expressions on a line become draw commands; trailing style tokens (positional `color=#hex` / `stroke=` / `fill=` / `thickness=<num>` / `label="..."`, or bare keywords `dashed` / `dotted` / `thick` / `thin` / `hidden`) decorate the draw. Hex literals (`#c0392b`) coexist with `#`-comments via lookahead: `#` followed by a hex digit is a color token, otherwise it starts a comment. `label P "..." pos=N|NE|E|SE|S|SW|W|NW` and `mark right_angle(P,Q,R)` / `mark angle(P,Q,R) arcs=N` / `mark tick(segment_expr, count=N)` cover annotations. The two flagship verification figures are the **nine-point circle** (three midpoints + three altitude feet + three Euler points all on a single `circle3(...)`) and **Pascal's theorem** (opposite-edge intersections of an inscribed hexagon are collinear with `line(X, Z)`); see `samples/kataskeve.md`. Output is a pure inline `<svg>` so HTML export needs no special snapshot handling (carries through like Schemata). Errors render as a `.kataskeve-error` red-bordered block at the failing block's location with the source line number; other blocks on the page render unaffected. Carries into Marp slides via the same `code[class*="language-"]` substitution loop, and `.kataskeve` is included in the Marp `marp-has-figs` figure-stage detection so geometry diagrams center vertically below leading text content.
...
- **Bundled starter themes**:
  ...
  - `tategaki.css` — Japanese vertical-writing theme (`writing-mode: vertical-rl`, mincho fonts, kinsoku 禁則 / tate-chu-yoko 縦中横 / palt 約物詰め, with `pre` / `table` / `img` / math / mermaid / schemata kept as horizontal islands; intentionally color-agnostic so it composes with both Light and Dark).
...
- **Extension inheritance inside slides**:
  ...
  - ` ```mermaid `, ` ```schemata `, and ` ```csv ` / ` ```tsv ` code blocks — post-render DOM substitution, reusing the same renderers as the regular pipeline.
...
- **Figure auto-centering** — when a non-split slide contains a `.mermaid` / `.schemata` / `.csv-table` / native `<table>` / `<video>` / `iframe.video-embed` / image-(or video-)only `<p>` (a paragraph whose sole meaningful child is `<img>` / `<video>` / `<iframe>`) as a direct child of `<section>`, a post-pass in `renderMarp()` wraps that element and every following sibling into `<div class="marp-figure-stage">` and tags the section with `marp-has-figs`. CSS then keeps the section a flex column (matching marp themes' own layout), pins `justify-content` to `flex-start` so leading text (heading + paragraphs) stays top-aligned, and gives the stage `flex: 1 1 auto` + inner `justify-content: center` so the figure(s) sit vertically centered within the remaining space (gap 16px). Horizontal centering is per-element (`text-align: center` on the mermaid/schemata/image wrapper, `margin: 0 auto` on tables/csv-tables) instead of `align-items: center` on the stage — `align-items: center` collapses the mermaid/schemata wrapper width and shrinks the inline SVG. Designed for the common "title + 1 line of context + diagram/table/image" layout — any content *after* the first figure also moves into the centered stage, so authors should keep the "text → figure" order. Split slides (`split` / `split-2` / `split-3` / `split-4`) are excluded since they use grid layout.
- **Column layout** (`<!-- _class: split -->` / `split-2` / `split-3` / `split-4`) — break a slide into 2 / 2 / 3 / 4 equal-width columns. Authors separate columns with a `+++` line at the start of a line; a `renderMarp()` pre-pass replaces each `+++` with a `⦃COL⦄` sentinel paragraph (skipping fenced code blocks so e.g. ```diff stays intact), and a post-pass groups direct children of `section[class*="split"]` into `<div class="split-col">` buckets at every sentinel `<p>`. Leading `<h1>` / `<h2>` of the first bucket are pulled out as a full-width header row (`grid-column: 1 / -1`) so all columns start at the same y. CSS applies `display: grid` with `repeat(N, 1fr)` and per-class gap. Runs after footnote restoration (so `<sup>` refs stay in their column) and before mermaid / schemata / csv / plotly substitution (so those land inside the correct column). The figure-centering pass explicitly skips these sections.
...
- `assets/libs/` — bundled JS/CSS: `marked`, `mermaid`, `katex/`, `highlight.js/`, `schemata.js`, `marp/` (Marp Core IIFE), `editor/` (CodeMirror 6 + Vim IIFE), `plotly/plotly.min.js` (Plotly.js full dist-min), `js-yaml.min.js` (YAML spec parser for `plotly` blocks).

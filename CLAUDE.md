...
| `assets/index.html` — a ` ```lang ` fence; `assets/libs/<engine>` | `.claude/docs/preview-blocks.md` | How do I add an engine? Why must it go through `awaitLib`? Why is the SVG cached without a theme salt? Why does `model3d` share one WebGL context and blit into a 2D canvas? Where does a tikz Japanese label get its colour from? |
...
| baseline `#preview { max-width: calc(var(--md-width-scale,1) * 900px) }` + its `@media print` twin — `assets/index.html` | `hakuro-modern.css`'s `40rem` cap **and its own `@media print` `min()`** and `tategaki.css`'s clamped `height` (`parchment` / `classical` inherit the baseline; `bunko` opts out on purpose). ⚠ A theme's `@media print` block is **later than the baseline's at equal specificity**, so a bare `max-width: 100%` there silently discards the multiplier on paper — hakuro did exactly that | partial (`keycheck.py` + `exportcheck.py`'s two `width_scale` cases + `pdfcheck.py`'s x-extent, now run per theme for 既定 + hakuro-modern; a theme that silently stopped consuming the variable would not be caught) |
...
| tikz / tikzcd — the `__tikzJpPreamble()` TeX macros, `__tikzFillJpText` / `__tikzAdoptJpFill`, `renderTikzCached` | `python tools/preview-harness/tikzcheck.py` (the ONLY check that can see tikz: `domdump.py` collapses the `<svg>`, and `shoot.py` counts a `.tikz-error` as settled) |
...
| `__TIKZ_BASE_PACKAGES` | `amsmath`, `amssymb` | the `\usepackage` lines **every** tikz/tikzcd block gets (`tikz-cd` is appended for `cd`). The core dump preloads standalone + inputenc + tikz/pgf and **no AMS layer**, so `\mathbb` / `\text{}` / `\dfrac` need it. ⚠ **Never re-split this by fence kind** — that asymmetry is exactly what made `\mathbb` work in `tikzcd` and fail in `tikz` with a misleading "check your TeX syntax". Detail: `preview-blocks.md` |
...
| `__FONT_SCALE_STEPS` | `[0.6 … 2.5]` | body-text `--md-font-scale` **anchor rungs** (`Ctrl` `+`/`-`/`0`) — ⚠ **not a range**: these are the familiar browser-zoom ratios, and `__scaleStep()` travels past both ends without limit. Reset is exact because 1 is an anchor. ⚠ Pinned to 1 under `--export-png`, or the PNG depends on the operator's localStorage |
| `__WIDTH_SCALE_STEPS` | `[0.6 … 2.0]` | content-width `--md-width-scale` anchor rungs (`S`-modal row only). The font anchors minus the top one: 2.5 × 900px is already off screen on every realistic window, so it need not be a *familiar* rung — travel continues past 2.0 regardless. Same ⚠ pin to 1 under `--export-png` — more load-bearing there, since this one moves the emitted PNG's pixel width. `tategaki` clamps it at 1.0 in CSS |
| `__SCALE_COARSE` / `__SCALE_SHRINK` | 0.25 / 0.9 | the rungs **outside** the anchors, shared by both scales. Additive going up (unbounded); multiplicative going down, so it approaches 0 without reaching it. Asymmetric on purpose: the useful enlarging range is linear, the useful shrinking range is not |
| `__SCALE_MIN` | 0.01 | ⚠ a **positivity guard, not a lower limit** — a 0× or negative multiplier is not a size. It is also the floor of `__scaleSanitize()`, which replaced anchor-membership validation once an arbitrary typed percentage became legitimate |

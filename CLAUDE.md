...
- **`S`-key Marp theme picker** — in Marp mode the same `S`-key modal repurposes itself to list selectable Marp themes (marp-core built-ins `default` / `gaia` / `uncover` plus every user theme discovered under `assets/marp/*.css`, name extracted from the `/* @theme <name> */` header). Selecting a row **rewrites the front-matter `theme:` line of the current Markdown file on disk** (in-place if the line exists, inserted after `marp: true` otherwise) — this is a real edit, not just a render-time override, so the choice persists across reloads and shows up if the file is opened in any other editor. The webview posts the new content via a `savefile:` IPC; the Rust handler mirrors `editor:save:` (`std::fs::write` + `suppressed_saves` to dodge the watcher) and additionally calls `editor_registry.push_file_to_editor` so a paired editor window's buffer stays in sync. Re-render rides the existing `EditorSavedContent` → `loadFileFromRust` path; the `default` choice writes an explicit `theme: default` line rather than removing the directive (clearer diff and intent).

...
### Cursor-block highlight (`cursorBlock`)

An opt-in tint on the preview block the editor's cursor is in. **It added no IPC and no `CustomEvent`**: the editor already posts `editor:cursor:<line>` from its `updateListener` on every `selectionSet && !docChanged`, and `editor:change:` already carries `line` — so both arrive at the preview as a function call and all this feature did was give that call something more to do.

- **`applyEditorCursor(line)` is the entry point, NOT `applyEditorScroll`.** The two Rust sites (`EditorCursorMoved`, and the concatenated `EditorLiveContent` script) were repointed at it. ⚠ The split is not decoration: `applyEditorScroll` has a second caller — the preview's own Back/Forward view restore (`assets/index.html`, `restoreLine`) — which has no editor behind it, so folding the tint in lit a block on `Alt+←` with no editor open and left it lit. `applyEditorCursor` owns `__editorCursorLine` and delegates the scroll; **`applyEditorCursor(0)` means "no editor cursor"** and clears without scrolling, which is the contract `clear_editor_cursor()` uses from all three editor-close arms in `src/main.rs` (window ×, `EditorCloseRequested`, `EditorCloseDecision` — skipped when `quit_app`, since the process is going away).
- **The pref is SHARED and UNPREFIXED — `cursorBlock`, not `editor:cursorBlock`.** It is written by this window's settings modal and read by the preview, which is the same origin in the same user-data folder; the same break with the `editor:*` convention `uiLang` makes, and for the same reason. Cross-window sync is the same `storage` + `focus` pair, with no `persist` flag needed because the preview never writes the key. ⚠ Serialization is **`'on'` / `'off'`** (the editor's `readPref` enum), not the preview's own `'true'` / `'false'` body-class convention — the writer's convention wins and the reader adapts. `prefscheck.py` pins the exact key name, because a rename produces a toggle that controls nothing, silently.
- **⚠ `:set cursorblock`, never `cursorline`.** This editor runs `@codemirror/view`'s `highlightActiveLine()`, so `:set nocursorline` already means "stop highlighting *my* caret line" to a Vim user. Named for what it does, like every other option in that switch.
- **Granularity is a block**, because `data-line` is stamped per top-level child and per `:scope > li`. The ja/en copy says ブロック / "block" deliberately; promising a line would be a promise the render pipeline cannot keep.
- Marp is excluded (`body.marp`): there `data-line` sits on the slide's whole `<svg>`, so the tint would cover an entire slide — every visible pixel in deck mode.

Preview-side detail — the shared `__blockForLine()` resolver, the two post-pagination re-apply sites, and why the highlighter must NOT guard on `__loadingFile` — is in `preview-core.md`. Nets: `synccheck.py` (all four themes, including the `bunko.css` split-paragraph case and the live-edit sequence), `exportcheck.py` (artifact + `@media print`), `prefscheck.py` (the modal row, the ex-commands, and `updateSettingsUI`).
...
All prefs persist via `localStorage` keys `editor:vim` (`on` / `off`), `editor:lineNumbers` (`absolute` / `relative` / `off`), `editor:theme` (`light` / `dark`), `editor:livePreview` (`on` / `off`), `editor:tableColHighlight` (`on` / `off`), `editor:tablePaste` (`on` / `off`), `editor:cellMode` (`on` / `off`), `editor:keyLayout` (`qwerty` / `dvorak`), `editor:fontSize` (a number — validated by `clampFontSize`, not the enum-only `readPref`) and `editor:fontFamily` (a key from `FONT_FAMILIES`, so the enum validator still applies and no arbitrary CSS string can ever be stored). ⚠ **One row of the modal does NOT persist under an `editor:`-prefixed key**: the cursor-block highlight uses the unprefixed, shared `cursorBlock` (`on` / `off`) because the preview is what reads it — see *Cursor-block highlight* above. Do not "fix" the inconsistency.

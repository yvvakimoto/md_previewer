...
```mermaid
flowchart LR
  A[入力<br/>.md ファイル / 関連画像] --> B[ホスト処理<br/>ファイル読込 / 相対パス解決 / 画像 base64 化]
  B --> C[描画<br/>marked / KaTeX / mermaid]
```
...
3. `marked` で AST を構築し、拡張をフックして数式・脚注のプレースホルダを差し込む
4. KaTeX が数式ノードを置換する
5. Mermaid が `<pre><code>` ブロックを SVG に展開する
...
Mermaid はコードフェンスから図を起こせるレンダラで、シーケンス図・フローチャート・ガントチャートなどに対応する。下記はドキュメント執筆ループを表したフローチャートの例。

```mermaid
flowchart LR
  A[構想<br/>目的整理 / 読者像] --> B[起草<br/>章立て / 初稿]
  B --> C[推敲<br/>査読 / 削る]
  C --> D[公開<br/>配布 / 反応収集]
  D --> A
```
...
`E` キーで「名前を付けて保存」ダイアログが開き、現在のレンダリング結果を **単一の HTML ファイル** として書き出す。SVG（Mermaid）と KaTeX のスパン、画像はすべてインライン化されるため、出力ファイルだけを社内 Web サーバや Confluence に貼っても見た目が崩れない。

...
> **補足（既知の制限）**: 通常文書の `page.png` は、文中に Mermaid 図があると高さ計測が過大になり下部に余白が付くことがあります（Marp スライドは固定 1280×720 なので影響を受けません — 本機能の主対象は Marp です）。撮影プロセスは連続起動せず 1 本ずつ順に実行してください。進捗・エラーは exe と同じ場所の `md-previewer.log` に記録されます。

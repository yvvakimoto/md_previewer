...
- **スライドの自動縮小（はみ出し時）** — 内容がスライド下端からはみ出すスライドは、既定で自動的に縮小されて 1 枚に収まります。**タイトル（H2）・ヘッダー・フッターは固定サイズのまま、本文エリアだけを縮小**するので、スライド間でタイトルの大きさが揃います（縮小率の下限は 0.5）。分割（`_class: split*`）／カラム（`::: columns`）スライドはレイアウトの都合でスライド全体を一律縮小。`A` キーで ON / OFF を切替（OFF のときは従来どおりはみ出し分をクリップ）。scroll / deck / list のどのモードでも、HTML エクスポートにも反映。[Marp のサンプル](samples/marp.md) 参照
...
| `A` | スライドの自動縮小 ON / OFF（はみ出し時、タイトル/ヘッダー/フッターは固定のまま本文だけを縮小して 1 枚に収める。既定 ON） |

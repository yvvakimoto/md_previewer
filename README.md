...
- テキスト選択可能なベクター出力、テーマの背景色も保持（A4 縦）
- **動画は静止画フレームに差し替え** — ローカル動画はその瞬間のフレームを画像化して PDF に載せます。フレーム時刻は alt の `|t=<時刻>` で指定（省略時は現在の再生位置。詳細は上の「動画 / YouTube 埋め込み」）

> **補足**: PDF 出力は **単一ファイル表示時のみ**（ワークスペースモードの `X` は従来どおり HTML フォルダ一括出力）。しおりの自動生成には比較的新しい WebView2 ランタイムが必要です（未対応環境ではしおり無しになりますが、リンクのクリックは常に機能します）。Marp スライドの PDF 最適化は今後の対応予定です。

...
- **スライドの自動縮小（はみ出し時）** — 内容がスライド下端からはみ出すスライドは、既定で自動的に一律縮小されて 1 枚に収まります（レイアウト・改行・図表・分割/カラム・背景を保ったまま均等に縮小、縮小率の下限は 0.5）。`A` キーで ON / OFF を切替（OFF のときは従来どおりはみ出し分をクリップ）。scroll / deck / list のどのモードでも、HTML エクスポートにも反映。[Marp のサンプル](samples/marp.md) 参照
...
| `A` | スライドの自動縮小 ON / OFF（内容がスライド下端からはみ出すとき、1 枚に収まるよう一律縮小。既定 ON） |

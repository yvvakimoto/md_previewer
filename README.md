...
`X` キーで **PDF** に書き出すと、**A4 横 1 枚に 2 ページ分**（見開き。1 ページ目が右）で印刷されます。図版などで幅の広くなったページは、自動的に 1 枚 1 ページに落ちるので切れません。
...
- **縦書きテーマは A4 横の複数ページ** — `tategaki.css` / `bunko.css` を選んでいるときは、用紙が **A4 横**になり、本文が右から左へ何ページにも分けて印刷されます。`bunko.css` は **1 枚に 2 ページ分（見開き。1 ページ目が右）**、`tategaki.css` は 1 行の長さも 1 ページの行数も**紙のサイズから自動的に**決まります。どちらもノンブル・しおり・リンク・テキスト選択はそのまま使えます

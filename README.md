...
- **文庫本レイアウト** — 同梱テーマ `bunko.css` を選ぶと、縦書き本文が **1行 39字 × 16行** のページに組まれ、ページの間に余白が入ります（段落は本物の本と同じようにページをまたいで分割）。読み進めは縦書きらしく横スクロール。字数・行数・行送り・書体などは `S` キー一覧の **⚙** から変更でき、動かすとその場で反映されます。⚙ の設定項目は CSS 側が `/* @user-vars */` で宣言する仕組みなので、**自作テーマにも同じように設定 UI を持たせられます**（後述の「テーマのカスタマイズ」を参照）
...
| `tategaki.css` | 日本語縦書きテーマ（禁則・縦中横・約物詰め。配色は指定しないのでライト／ダークどちらとも合成可） |
...
字数・行数はもちろん、文字サイズ・行送り・天地の余白・ページ間の余白・段落の字下げ・書体・約物の詰めを、後述の ⚙ から変更できます。変更すると自動的にページを割り直します。

> 画像・コードブロック・表・数式は大きさが行の整数倍にならないため、それを含むページだけ行数が足りなくなります（文庫レイアウトは元来、地の文のためのものです）。
...
**設定できる項目はスタイルによって異なります**（`bunko.css` は 9 項目、`tategaki.css` は 3 項目、その他の同梱テーマは設定項目なし＝⚙ も出ません）。項目は CSS 側が自分で宣言する仕組みで、自作テーマにも同じように付けられます。CSS のどこか（慣例としてファイル末尾）に次のようなコメントを書くだけです。
...
> **補足**: PDF 出力は **単一ファイル表示時のみ**（ワークスペースモードの `X` は従来どおり HTML フォルダ一括出力）。しおりの自動生成には比較的新しい WebView2 ランタイムが必要です（未対応環境ではしおり無しになりますが、リンクのクリックは常に機能します）。オートフィット（`A` キー）の下限まで縮めても入りきらないスライドは、画面表示と同じくはみ出した部分が切れます。

...
- **表（テーブル）支援** — **雛形の挿入**: 行数・列数（と揃え）を指定して GFM のパイプ表を挿入します。行数は見出し行を含む数です。**カーソル列のハイライト**: カーソルがあるセルの列を、その表の全行にわたって淡く強調します（見出し行も対象、区切り行は対象外）。呼び出しは、ステータスバーのボタン（`⊞ Table` / `Col: ON`・`Col: OFF`）／ Vim の ex コマンド `:table` `:tbl` `:tablecol`（`:table 3 4`・`:table 3x4`・`:table 4`（正方）・`:table 3 4 center` など。引数なしはモーダル）／ `:set tablecolumn` `:set notablecolumn`（`tcol` / `notcol`）／ Vim NORMAL の `gti`（挿入）`gtc`（ハイライト切替）／ 非 Vim の `Ctrl+Alt+T` `Ctrl+Alt+H`。ハイライトの ON/OFF は `localStorage` に永続化されます（既定 ON）。
...
- **セルモード（Jupyter 準拠）** — `⌗ Cells: ON` にすると **`---` 区切りが「セル」** として扱われ、Jupyter と同じ操作感で塊ごとに編集できます。Marp 文書に限らず、すべての `.md` で使えます（`---` はスライド境界と同じ単位なので、Marp では**セル N = スライド N**）。
  ...
  - **呼び出し** — ステータスバーの `⌗ Cells` ボタン ／ Vim の ex コマンド `:cellmode`・`:set cells` / `:set nocells` ／ Vim NORMAL の `gmc`。設定は `localStorage` に永続化（既定 **OFF**）で、OFF に戻せば装飾・セル番号・キー割り当てはすべて消え、従来のテキスト編集に戻ります。[セルモードのサンプル](samples/cells.md) 参照
- **ステータスバーのトグル** — 上端ホバーで現れるバーから **Vim ON/OFF**・**行番号（絶対 / 相対 / OFF）**・**テーマ（Light / Dark）**・**ライブプレビュー ON/OFF**・**表の列ハイライト ON/OFF**・**セルモード ON/OFF** を切替。設定はいずれも `localStorage` に永続化されます。

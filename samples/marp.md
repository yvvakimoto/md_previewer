...
1 枚に収まりきらないほど内容が多いスライドは、`A` キーが ON のとき（既定）自動縮小されて全体が 1 枚に収まります。`A` キーで OFF にすると従来どおり下端で切り取られます。
...
- 行 3: 縮小率の下限は 0.5 で、それ以上はみ出す分は従来どおりクリップされます。分割 / カラムスライドはレイアウトの都合でスライド全体を一律縮小します。

...
**markwhen** ブロックは図として中央に配置されます（`marp-has-figs`）。
...
1 枚に収まりきらないほど内容が多いスライドは、`A` キーが ON のとき（既定）自動で一律縮小されて全体が 1 枚に収まります。`A` キーで OFF にすると従来どおり下端で切り取られます。

- 行 1: 長文の箇条書きをたくさん並べても、下端からはみ出さずに収まります。
- 行 2: 縮小はレイアウト・改行・図表・縦位置・背景・分割/カラムを保ったまま均等に行われます。
- 行 3: 縮小率の下限は 0.5 で、それ以上はみ出す分は従来どおりクリップされます。
- 行 4: スクロール / デッキ / リストのどのモードでも同じ見た目になります。

...
**model3d** ブロックは外部の 3D メッシュファイル（STL / OBJ / PLY / glTF / GLB / 3MF）を読み込んで描画し、図として左右中央に配置されます。スライド上でもドラッグ回転・ホイールズームができます。
...
サイズはフェンスの `width:` / `height:` で決まり、**レイアウトからは一切測りません**。だからスクロール／デッキ／一覧のどの表示でも、また HTML・PDF・PNG のどのエクスポートでも同じ大きさで出ます。

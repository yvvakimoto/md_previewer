...
3. `marked` で AST を構築し、拡張をフックして数式・脚注・SmartArt のプレースホルダを差し込む
4. KaTeX が数式ノードを置換する
5. Mermaid と SmartArt が `<pre><code>` ブロックを SVG に展開する
...
Mermaid と SmartArt は別エンジンだが、UI 上は同じ「コードフェンスから図」の体験を提供する。Mermaid は標準的なシーケンス図・フローチャート・ガントチャートに強く、SmartArt はビジネス文書で多用される特性要因図・PDCA サイクル・BMC・マンダラチャートを補う。
...
`E` キーで「名前を付けて保存」ダイアログが開き、現在のレンダリング結果を **単一の HTML ファイル** として書き出す。SVG（Mermaid・SmartArt）と KaTeX のスパン、画像はすべてインライン化されるため、出力ファイルだけを社内 Web サーバや Confluence に貼っても見た目が崩れない。
